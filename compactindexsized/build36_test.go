...
func TestBuilder36(t *testing.T) {
	const numBuckets = 3
	const valueSize = 36
	...
	expected := concatBytes(
		...
		[]byte{0xe2, 0xdb, 0x55},
		// value
		[]byte{0x1, 0x71, 0x12, 0x20, 0x20, 0xea, 0xb3, 0xf, 0x58, 0xbe, 0x69, 0x1, 0x7f, 0x2, 0x42, 0x91, 0xfa, 0xa3, 0xdc, 0xf4, 0xc7, 0xf2, 0x2f, 0x56, 0x12, 0xa7, 0xb2, 0x1, 0x6f, 0x48, 0xfa, 0x17, 0x5e, 0x53, 0xda, 0x6b},

		// --- Bucket 2
		// hash
		[]byte{0x92, 0xcd, 0xbb},
		// value
		[]byte{0x01, 0x71, 0x12, 0x20, 0x9c, 0xd0, 0x17, 0x9a, 0x19, 0x9c, 0xd9, 0x51, 0x0a, 0xfb, 0x92, 0x96, 0xcf, 0xd2, 0x9f, 0x77, 0x8a, 0x00, 0x40, 0x32, 0x8b, 0xf8, 0xff, 0x06, 0x46, 0x21, 0xb9, 0x3c, 0x57, 0xa5, 0xdd, 0x0f},
		// hash
		[]byte{0x98, 0x3d, 0xbd},
		// value
		[]byte{0x01, 0x71, 0x12, 0x20, 0x1b, 0x79, 0x02, 0x6c, 0x3d, 0xdc, 0x74, 0x0c, 0x33, 0x71, 0xf0, 0x7a, 0x4b, 0x80, 0xb0, 0x43, 0x0c, 0x82, 0x0a, 0x88, 0x72, 0x13, 0xa6, 0x94, 0x72, 0xc9, 0xd1, 0x8a, 0x2d, 0xc7, 0x88, 0x13},
		// hash
		[]byte{0xe3, 0x09, 0x6b},
		// value
		[]byte{0x1, 0x71, 0x12, 0x20, 0x60, 0x67, 0x54, 0xe4, 0x4c, 0x5, 0x99, 0x6f, 0xf9, 0x60, 0x66, 0x27, 0x66, 0xd, 0xa0, 0xda, 0x4f, 0x60, 0x10, 0x6, 0x2, 0x82, 0xf9, 0x46, 0x3d, 0xcc, 0xde, 0x28, 0x80, 0x72, 0x41, 0x67},
	)
	...
	assert.Equal(t, []Entry{
		{
			Hash:  12402072,
			Value: []byte(testCids[3].Bytes()),
		},
		{
			Hash:  7014883,
			Value: []byte(testCids[2].Bytes()),
		},
	}, entries)

	{
		for i, keyString := range keys {
			...
			require.NoError(t, err)
			assert.Equal(t, []byte(testCids[i].Bytes()), value)
		}
	}
}

func TestBuilder36_Random(t *testing.T) {
	if testing.Short() {
	...
	for i := uint(0); i < numKeys; i++ {
		...
		err := builder.Insert(key, []byte(testCids[i].Bytes()))
		require.NoError(t, err)
	}
	...
	for i := queries; i != 0; i-- {
		...
		value, err := bucket.Lookup(key)
		require.NoError(t, err)
		require.Equal(t, []byte(testCids[keyN].Bytes()), value)
	}
