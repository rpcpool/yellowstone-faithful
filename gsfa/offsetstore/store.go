...
import (
	...
	storetypes "github.com/rpcpool/yellowstone-faithful/gsfa/store/types"
)
...
func OpenOffsetStore(ctx context.Context, indexPath string, dataPath string, options ...store.Option) (*OffsetStore, error) {
...
func (as *OffsetStore) DeleteAccount(ctx context.Context, pk solana.PublicKey) error {
	if ctx.Err() != nil {
		return ctx.Err()
	}
