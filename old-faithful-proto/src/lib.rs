...
pub mod decode {
    ...
    pub fn confirmed_transaction(
        ...
        let index = tx
            .index
            .or(transaction.index)
            .unwrap_or_default()
            .try_into()
            .context("transaction index exceeds u32")?;
        versioned_transaction(transaction).map(|tx_with_meta| ConfirmedTransactionWithStatusMeta {
            slot: tx.slot,
            tx_with_meta,
    ...
    pub fn versioned_transaction(
        ...
        let transaction: VersionedTransaction = match wincode::deserialize(&tx.transaction) {
            Ok(transaction) => transaction,
            Err(wincode_error) => bincode::deserialize(&tx.transaction).with_context(|| {
                format!(
                    "failed to decode VersionedTransaction with wincode ({wincode_error}) or bincode"
                )
            })?,
        };
